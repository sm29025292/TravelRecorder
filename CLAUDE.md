...
- ✅ **T38 分帳頁：同行者手機卡片＋結餘表壓縮＋護照名 placeholder 移除**（2026-07-19）：全部在
  `src/components/trip/SettlementTab.tsx`（凍結區局部解凍：僅版面層；`settle`／`settleByCurrency`／
  expenses 彙整迴圈／`MemberSelect`／`ParticipantsPicker` 全未動）。①**placeholder 移除**：
  護照名 `TextInput` 的 `placeholder="LIN,LIWEN"` 屬性整個刪除（桌面表格版）。②**同行者手機卡片**
  （模式照 T31／T32／共通守則）：頂層加 `expandedIds: Set<string>` state ＋ `toggleExpand(id)`
  （抄 `ItineraryTab.tsx` 手法）；`addMember` 於 `db.members.add(m)` 之後把 `m.id` 加入
  `expandedIds`（新增即展開）。外層 `<div className="overflow-x-auto rounded-lg border bg-white">`
  拆為 `rounded-lg border bg-white` 容器＋桌面 `<div className="hidden overflow-x-auto sm:block">`
  （原表格 `min-w-[40rem]` 照舊）＋手機 `<div className="divide-y sm:hidden">`（卡片群）。
  卡片摘要無內部互動元素 → 用 `<button type="button" aria-expanded={expanded}
  onClick={() => toggleExpand(m.id)}` 即可（不需 `div role="button"`）：`m.name` 空 → 灰字「(未命名)」、
  `flex-1 truncate`；`m.passportName` 非空才顯示 `shrink-0 text-xs text-gray-500`；▼／▲ 最右。
  展開後直向表單（沿用檔尾新增的 local `CardField`）：姓名 `TextInput`、護照名 `TextInput`（無 placeholder）、
  護照號碼 `TextInput`、生日 `DateInput`，右下 `✕ 刪除這位成員` 按鈕（呼叫既有 `removeM`；樣式抄
  `ItineraryTab` 卡片刪除鈕：`rounded px-2 py-1 text-xs text-red-600 hover:bg-red-50`）。空狀態雙容器：
  桌面既有 `colSpan={5}` 列照舊、手機置中灰字同文案。③**結餘表壓縮**：結餘表
  `<table className="w-full min-w-[32rem] text-sm">` 改 `<table className="w-full text-sm sm:min-w-[32rem]">`
  ——手機斷點下無最小寬度、「成員／已付／應分攤／結餘」四欄自然壓縮同屏；表格內容、`fmt`、紅綠結餘樣式
  全部不動。「結算建議」文字清單本來就是文字、不動。檔尾新增 local `CardField` 元件（抄 `ItineraryTab.tsx`
  檔尾：`w-14` 標籤＋`flex-1` 欄位）。React import 由 `useLiveQuery` 一行擴充為
  `import { useState, type ReactNode } from 'react'`。全 155 綠、build 通過。
- ✅ **T37 行程手機卡片摘要改版（金額→備註＋連結）**（2026-07-18）：全部在
  `src/components/trip/ItineraryTab.tsx` 的 `renderCard`（桌面 `renderRow`／`renderHead`／
  頁尾總計／`timeSummary`／`attractionName`／`expandedIds`／`toggleExpand` 全未動）。
  摘要列外層由 `<button>` 改
  `<div role="button" tabIndex={0} aria-expanded={expanded} onClick={() => toggleExpand(it.id)}
   onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { if (e.key === ' ') e.preventDefault();
   toggleExpand(it.id) } }} className="cursor-pointer text-sm hover:bg-gray-50">`——原因：摘要列
  內部要放 `<a>` 連結，HTML 禁止巢狀互動元素、React 也會警告，改用 `role="button"` 的 div
  同時保留鍵盤可及性（Enter／空白觸發展開，空白 `preventDefault` 防捲動）。原 `flex ... px-3 py-2`
  下移到內層新增的「主行 `<div className="flex items-center gap-2 px-3 py-2">`」，並在其後加
  optional 備註行 `<div className="truncate px-3 pb-2 text-xs text-gray-400">{it.notes}</div>`
  （`it.notes` 空則不渲染、主行自己的 `py-2` 維持卡片高度；有備註時 `pb-2` 追加下內距）。
  主行內容：時間（既有 `timeSummary`）＋景點名（既有 `attractionName`）**照舊**；**移除**
  右側「時數 · 外幣小計」`<span>`（原 `it.hours ? ... foreign ...` 那段），改在同位置放
  連結——`const p = parseLink(it.link)`（`parseLink` 自 `../../lib/link` import），`p.url` 非空
  才渲染 `<a href={p.url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()}
   className="max-w-[7.5rem] shrink-0 truncate text-xs text-sky-600 underline decoration-sky-300
   underline-offset-2">{p.text || p.url}</a>`——點連結開新分頁**且卡片不展開**（`stopPropagation`），
  摘要列不放 ✎（編輯入口在展開後的 LinkField）；▼／▲ 三角照舊放最右。展開後直向表單完全不動
  （T35 已把連結欄換 LinkField；小計行本來就在展開區、金額資訊不流失）；`foreign` 計算保留
  （仍被展開區的小計行使用）。旅途中看行程要的是地點／備註／連結，錢展開再看即可——已拍板。
  無新測試（純 UI／版面調整，`parseLink` 已有 T35 測試涵蓋）；全 155 綠、build 通過。
- ✅ **T39 行李頁手機卡片**（2026-07-19）：`src/components/trip/PackingTab.tsx` 沿用 T31／T32／T38 模式。
  頂層加 `expandedIds: Set<string>` state ＋ `toggleExpand(id)`（抄 `ItineraryTab.tsx` 手法）；
  `addRow` 於 `db.packing.add(row)` 之後把 `row.id` 加入 `expandedIds`（新增即展開）。`renderRow`
  由原本內嵌整段 `<tr>` 抽成函式（表格版 markup 完全不動）；新增 `renderCard(it)`——摘要列
  **內含 checkbox（互動元素），外層用 `<div role="button" tabIndex={0} aria-expanded=... onClick=...
  onKeyDown=...>`**（Enter／空白鍵支援、空白 `preventDefault` 防捲動；共通守則明文規範）：
  勾選框沿用桌面同款 `<input type="checkbox" className="h-4 w-4 shrink-0 accent-sky-600" ...>`
  ＋另加 `onClick={(e) => e.stopPropagation()}`——**收合狀態直接點勾選框可打勾／取消、卡片不展開**
  （已拍板：邊收行李邊打勾是最常見動作、免展開）；項目名 `flex-1 truncate`（空→灰字「(未填)」、
  `it.checked` 時同時加 `text-gray-400 line-through` 比照桌面）；`it.quantity !== 0` 時右側灰字
  小字（`shrink-0 text-xs text-gray-500`）顯示數字（不加單位；零＝不顯示，與全 App「數字 0 顯示
  空白」慣例一致）；▼／▲ 最右。`it.notes` 非空 → 主行下方備註行
  `<div className="truncate px-3 pb-2 text-xs text-gray-400">`（版型同 T37）。展開後直向表單：
  項目（`TextInput`，`checked` 時 className 同樣加劃線灰字）、份量(人)（`NumberInput`）、備註
  （`TextInput`）、右下 `✕ 刪除這列`（樣式抄 `ItineraryTab` 卡片刪除鈕）。斷點：Tailwind `sm`
  （640px）——原 `<div className="overflow-x-auto rounded-lg border bg-white">` 拆為外層
  `rounded-lg border bg-white` 容器＋內層桌面 `hidden overflow-x-auto sm:block`（原表格 `min-w-[36rem]`
  照舊）＋內層手機 `divide-y sm:hidden`（新卡片群）；空狀態雙容器（桌面 `<tr colSpan={5}>`＋手機
  置中文字）避免手機被 `min-w-[36rem]` 撐出橫向捲動。頁尾「＋新增一列」與「已勾選 X / Y」照舊
  （既有 flex-wrap 版面不用改）。局部元件 `CardField`（`w-14` 標籤＋`flex-1` 欄位）擺 `PackingTab.tsx`
...
- ✅ **T47 景點庫新增表單：按鈕移到最下方獨立一列＋手機兩欄順序重排**（2026-09-09）：全部在
  ...
  整組移出 grid，改放 grid 之後的 `<div className="mt-3 flex items-center justify-start gap-2">`
  ——**按鈕靠左自成一列、提示在按鈕右側同一列**（取代 T44 第二輪「塞進九宮格第一排最右格」的做法，
  該格視覺上像一個欄位、與輸入框混淆；靠左／提示在右是 2026-09-09 擁有者看桌面截圖後拍板，
  取代同日稍早先做的「靠右／提示在左」）。②DOM 順序把「景點名稱」往前搬到「類型」之後，於是
  ...
  ＋連結框 206／236px 同一行、按鈕在 grid 下方（靠左，左緣距表單左緣 13px＝容器內距）、提示在按鈕
  右側同列、整頁 `scrollWidth === clientWidth`（零橫向溢出）；1400px 桌面三排 5 欄、按鈕同樣靠左；
  實際新增一筆（含網址名稱＋連結）確認國家/都市/區域保留、其餘欄位清空。
...
- **範疇決策**：
  ...
  - **第四輪定案（2026-07-17，T34–T39，規格詳見 ISSUE_LIST.md）**：
    ...
    **連結超連結化**——`ItineraryItem.link` 沿用單一字串但支援 `[名稱](網址)` 編碼
    ...
    第二行灰字備註；金額只在展開後；摘要列因內含 `<a>` 改 `div role="button"`；
    T37 ✅ 2026-07-18，`ItineraryTab.tsx` 的 `renderCard` 摘要列外層改 `div role="button"`＋
    `onKeyDown` Enter/Space 支援、主行右側時數/外幣 `<span>` 換成 `parseLink(it.link)` 產生的
    可點超連結（`stopPropagation` 免展開）、主行下方 optional 備註行截斷灰字；展開後表單／
    桌面表格／頁尾總計全未動）；
    **分帳／行李凍結區局部解凍（版面層）**——分帳頁同行者手機卡片（收合只顯示中英名
    「姓名＋護照名」）＋結餘表手機拿掉 `min-w` 自然壓縮＋護照名 placeholder「LIN,LIWEN」移除
    （T38 ✅ 2026-07-19，`SettlementTab.tsx` 加 `expandedIds`／`toggleExpand`／
    `addMember` 新增即展開；外層雙容器桌面表格＋手機卡片；卡片摘要無互動元素故用 `<button>`；
    展開後直向表單四欄＋刪除；結餘表 `min-w-[32rem]` → `sm:min-w-[32rem]`；`settle`／
    `settleByCurrency`／entries 彙整迴圈／`MemberSelect`／`ParticipantsPicker` 全未動）；
    行李頁手機卡片（已拍板互動分工：**勾選框收合直接可點打勾（stopPropagation）、
    點列其他處展開編輯**；T39 ✅ 2026-07-19，`PackingTab.tsx` 加 `expandedIds`／`toggleExpand`／
    `renderCard`、`renderRow` 抽函式、表格外容器拆桌面／手機雙渲染；摘要列因內含 checkbox 用
  ...
  - **T46 行程景點下拉加入「出發地國家」**（✅ 2026-09-09，實作摘要見 §7）：
    ...
    `AttractionPicker` 加 optional prop `originCountry`，國家過濾由「一國」變「目的地＋出發地兩國」，
  ...
  - **T48 景點列表手機卡片式檢視**（✅ 2026-09-09，實作摘要見 §7）：
    ...
    手機卡片群，頂層加 `expandedIds`／`toggleExpand`（**不可**放 `renderRows` 內，該函式一頁多次呼叫），
    摘要列無互動元素故用 `<button>`（名稱＋✓＋★＋類型標籤＋▼），展開後編景點名稱／詳細地址／
    網址（LinkField）／備註／優先度／類型＋「搬移」「✕ 刪除這個景點」。呼叫端本就只在非空清單
    呼叫，不需空狀態雙容器；`update`／`remove`（含 T5 引用數防護）／`openMoveRow` 全共用不動。
  - **先擱置（2026-09-09）**：**行程列手動調整順序**——與 T11「同日依 `time` 自動排序」天生衝突，
    已在對話中討論三案（(a) 同日排序鍵改回 `sort`＋每列 ↑↓＋「依時間排序」按鈕＋既有 `sort` 一次性
    依目前顯示順序重寫；(b) `Trip` 加「排序：時間／手動」模式開關；(c) 拖曳排序），
    擁有者決定先擱置，重啟時再擇一拍板（三案取捨記錄於 ISSUE_LIST.md「觀察中／不做」）。
...
- **新需求／功能修改，走以下流程再進 `main`**：
  1. 有新功能需求或修改想法時，先**在對話中討論並確認需求內容**（範圍、設計決定），不要邊做邊定義需求。
  2. 需求確認後，依**功能大小與相依性**評估、調整任務分類（是否拆成多個子任務、標上任務編號 `T##`、
     註明依賴的前置任務），比照現有 `ISSUE_LIST.md` 的寫法（一個 session 接手一個任務）。
  3. 任務分類**經開發者在對話中確認**後，才把任務寫進 `ISSUE_LIST.md` 的進度表／規格——這一步是
     **純文件變更**，直接對 `main` commit + push（GitHub 網頁編輯，或本機 `git checkout main &&
     git pull && 編輯 → git commit && git push origin main`）；`main` 若有分支保護擋直接 push，
     改開一個只改 `ISSUE_LIST.md` 的小 PR、開了立刻自行合併。這樣不論誰在哪條分支開發，都能在
     `main` 立刻看到待辦事項，且待辦事項一定是已確認過的，不會有單方面塞需求的狀況。
  4. **實作**任務的程式碼時，才切到該任務指定的**功能分支**開發；commit 後推送到該分支，
     **完成並驗證（`npm run test` / `npm run build` 全綠）後才合併回 `main`**。
     **未經同意不要 push 到別的分支、不要開 PR。**
  5. 完成後：把該任務在 `ISSUE_LIST.md` 進度總覽表的狀態改 ✅（加日期），並**把「最新狀況」更新
     回本檔 CLAUDE.md**（見下方 Stop hook 說明）。
