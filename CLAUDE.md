...
| 路徑 | 用途 |
|------|------|
| `ISSUE_LIST.md` | **開發任務清單**（進行中 T25–T33 的完整規格、依賴、驗收條件與進度表，供後續 session 逐項接手；T1–T24 已完成歸檔，完成摘要見本檔 §7） |
...
| `src/components/AttractionPicker.tsx` | 行程用的景點下拉（T16 三段式：類型／都市內建篩選、國家由旅程鎖死；optgroup label 隨篩選狀態自組；含「目前選取」保底 optgroup 讓 `<select>` 永不空白） |
...
- ✅ **單元測試 116 項**：`money`(23，含 `settle` 分帳＋T12 `itineraryForeignSubtotal`＋T24 成對淨額/`settleByCurrency`) + `csv`(5) + `importAttractions`(12) + `migrate`(5) + `currency`(5) + `itinerary`(29，T6 分組／週幾／當日小計 + T11 組內時間排序 + T13 range 補空日／`datesInRange` + T18 `hoursBetween`) + `group`(7，T4 `buildLocationTree` + T7 組內 priority 排序) + `dedupeAttractions`(11，T8 `normalizeName`/`findDuplicateGroups`/`mergeAttractionFields`) + `orphanItinerary`(5，T9 `findOrphanItinerary`) + `visited`(3，T15 `visitedAttractionIds`) + `exportItinerary`(6，T22 `itineraryToText`) + `crypto`(5，T10 roundtrip／錯誤密語／salt+iv 隨機／envelope 欄位／壞 JSON)，`npm run test` 全綠。
...
**先讀本檔、再挑 ISSUE_LIST.md 的任務執行**。第一、二輪 T1–T24 已全數完成（規格歸檔，見 §7）；
現行為**第三輪 T25–T33**（2026-07-12 與擁有者討論定案，設計決定已拍板、勿重新設計）。
...
- **範疇決策**：
  ...
  - **第三輪定案（2026-07-12，T25–T33，規格詳見 ISSUE_LIST.md）**：
    同步——SyncDialog 下載按鈕文字修正＋備份 `version` 升 5（T25）、Gist 上傳防覆蓋保護
    （上傳前比對雲端 `updated_at` 與本機 `tr.sync.lastSyncedAt`，不同即 confirm；T26）；
    輸入元件——時間輸入改 24 小時制純文字＋失焦正規化（`930`→`09:30`；已拍板放棄原生時間
    滾輪換取保證 24h 與窄欄寬，T27）、數字輸入以全域 CSS 隱藏上下箭頭（T28）；
    行程頁——移除「行程／活動」欄＋AttractionPicker 三下拉（類型／都市／景點）拆成獨立表格欄
    （已拍板：`activity` 資料欄位保留、T22 匯出對舊資料照常輸出，僅 UI 移除；T29）、
    旅程日期整批平移（trip 起訖＋行程列日期；花費日期不動——付款日是歷史事實；T30）；
    **手機卡片式檢視（本輪重點投資，擁有者特別強調）**——行程頁先行（T31，`sm` 以下卡片、
    桌面表格保留、邏輯共用不複製）、花費頁接續（T32，沿用 T31 模式）；
    健檢擴充成員孤兒參照（T33，P3 低優先、凍結區局部解凍）。
