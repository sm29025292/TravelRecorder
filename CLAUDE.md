...
- ✅ **單元測試 50 項**：`money`(16，含 `settle` 分帳) + `csv`(5) + `importAttractions`(5) + `migrate`(5) + `currency`(5) + `itinerary`(9，T6) + `group`(5，T4 `buildLocationTree`)，`npm run test` 全綠。
...
- **範疇決策**：
  ...
  - **景點庫強化剩餘**（P2）：刪除防護（T5）、開網址/名稱搜尋/優先度星星（T7）。
