...
- ✅ **單元測試 63 項**：`money`(16，含 `settle` 分帳) + `csv`(5) + `importAttractions`(5) + `migrate`(5) + `currency`(5) + `itinerary`(9，T6) + `group`(7，T4 `buildLocationTree` + T7 組內 priority 排序) + `dedupeAttractions`(11，T8 `normalizeName`/`findDuplicateGroups`/`mergeAttractionFields`)，`npm run test` 全綠。
