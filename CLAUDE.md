...
- ⚠️ **預設分支目前是 `claude/hopeful-johnson-7dkkix`，不是 `main`**（`main` 通常同步到同一 commit）。
  `github-pages` 環境的保護規則**只允許從「預設分支」部署**，所以：
  - 目前要部署 → 在**預設分支**上 `workflow_dispatch`（GitHub MCP `actions_run_trigger` 的 `run_workflow`），再用 Actions API 確認 build+deploy 皆 success。
  - ⚠️ workflow 有 `concurrency: { group: pages, cancel-in-progress: true }`：**push `main` 會觸發一個因環境保護而失敗的 run，並可能取消正在跑的 dispatch**；別同時 push `main` 與觸發部署，純文件變更可在 commit 訊息加 `[skip ci]`。
  - 建議改善：把預設分支設成 `main`，之後 push `main` 就會自動部署，也免去上述雜訊。
