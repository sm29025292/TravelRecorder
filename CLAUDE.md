...
- ✅ **單元測試 110 項**：`money`(23，含 `settle` 分帳＋T12 `itineraryForeignSubtotal`＋T24 成對淨額/`settleByCurrency`) + `csv`(5) + `importAttractions`(12) + `migrate`(5) + `currency`(5) + `itinerary`(29，T6 分組／週幾／當日小計 + T11 組內時間排序 + T13 range 補空日／`datesInRange` + T18 `hoursBetween`) + `group`(7，T4 `buildLocationTree` + T7 組內 priority 排序) + `dedupeAttractions`(11，T8 `normalizeName`/`findDuplicateGroups`/`mergeAttractionFields`) + `orphanItinerary`(5，T9 `findOrphanItinerary`) + `visited`(3，T15 `visitedAttractionIds`) + `crypto`(5，T10 roundtrip／錯誤密語／salt+iv 隨機／envelope 欄位／壞 JSON)，`npm run test` 全綠。
...
- **範疇決策**：
  ...
  - **第二輪定案（2026-07-07，T11–T23，規格詳見 ISSUE_LIST.md）**：
    ...
    起訖時間 `endTime`＋時數自動計算（T18 ✅ 2026-07-08）、逐日行程匯出文字（T22）；
