...
> 本檔只保留其進度列。現行任務為 **T11–T24**（T11–T23 於 2026-07-07 第二輪討論定案；
> T24 於 2026-07-08 與擁有者討論增補）。
...
### 進行中（T11–T24，表列順序＝建議施工順序）

| # | 任務 | 優先 | 難度 | 依賴 | 狀態 |
|---|------|------|------|------|------|
| T11 | 行程同日依時間排序＋時間/日期輸入失焦寫入 | P1 | ★★ | 無 | ✅ 2026-07-07 |
| T12 | 行程列小計改外幣為主＋頁尾三數值總計 | P1 | ★ | 建議 T11 後（同檔） | ✅ 2026-07-07 |
| T13 | 行程頁依旅程日期自動列出所有天數 | P1 | ★★ | 建議 T11 後（同函式） | ✅ 2026-07-08 |
| T14 | 景點類型擴充「住宿」「交通」 | P1 | ★ | 無 | ✅ 2026-07-08 |
| T15 | 景點「已去過」標記 | P2 | ★★ | 無 | ✅ 2026-07-08 |
| T16 | AttractionPicker 三段式改造（類型→都市→景點、國家鎖死） | P1 | ★★★ | T14；建議 T15 後（同元件） | ✅ 2026-07-10 |
| T17 | 行程移除付錢/分攤欄＋分帳結算不再計入行程 | P1 | ★★ | 無 | ✅ 2026-07-08 |
| T18 | 行程起訖時間（endTime＋時數自動計算） | P1 | ★★ | 建議 T11/T12/T17 後（同檔） | ✅ 2026-07-08 |
| T19 | 花費「已結清」生效（結算排除） | P1 | ★ | T17 | ✅ 2026-07-08 |
| T20 | 總覽移除人數欄＋花費平均改用成員數 | P1 | ★ | 無 | ✅ 2026-07-08 |
| T21 | 行李清單繼承上次旅程 | P2 | ★ | 無 | ✅ 2026-07-10 |
| T22 | 逐日行程匯出文字（複製到剪貼簿） | P2 | ★★ | T18 | ✅ 2026-07-10 |
| T23 | 匯入備份可選「取代／合併」 | P2 | ★★ | 無 | ✅ 2026-07-10 |
| T24 | 分帳成對淨額＋各幣別獨立結算 | P1 | ★★ | T17、T19 | ✅ 2026-07-08 |
...
- **凍結區（局部解凍）**：分帳／成員（`SettlementTab.tsx`、`money.ts` 的 `settle`、`MemberSelect.tsx`、
  ...
  **但 T17／T19／T20／T21／T24 的規格明文要求修改的部分可以動**——僅限各該任務點名的檔案與範圍，
...
- Dexie 目前 schema 版本為 **v5**。**新增「無索引」欄位不需要升版**（Dexie 物件是 schemaless，
  索引才需要宣告）——T18 加 `endTime` 即屬此類；T11–T24 **沒有任何任務需要升 Dexie 版本**。
...
## T24 分帳成對淨額＋各幣別獨立結算（P1・★★・依賴：T17、T19）✅ 2026-07-08

**背景／目標**（2026-07-08 與擁有者討論拍板）：
1. 原 `settle()` 用貪婪法追求「最少轉帳筆數」，會把債務改道——例：A 付 1200、B 付 300（三人均分）
   → 貪婪法給「C→A 500、B→A 200」，但 C 實際欠 A 400、欠 B 100。擁有者期望**成對淨額**
   （兩兩互欠對沖）：「B→A 300、C→A 400、C→B 100」——每筆轉帳都對應實際債務、可追溯，
   筆數可能多於全域最少（已拍板：直觀優先，不保留貪婪法、不做模式切換）。
2. 花費列可選台幣或外幣，原結算把外幣列以旅程匯率換成台幣再算；改為**各幣別獨立結算、
   完全不換匯、不跨幣別對沖**（已拍板）。「手續費(元)」欄是台幣 → 外幣列的手續費
   另計一筆**進台幣桶**（同付錢者、同分攤；已拍板）。

**規格**：
1. `src/lib/money.ts` `settle()`：balances 計算不動；entries 迴圈同時累計 debtor→creditor 債務
   （付錢者自付的份不記債），轉帳產生改成對淨額（每對成員互欠對沖、依 memberIds 順序輸出）。
2. `src/lib/money.ts` 新增 `settleByCurrency(memberIds, entries)`：entry 多 `currency` 欄
   （空字串視為 TWD），按幣別分桶各自跑 `settle`；TWD 桶排最前、其餘依出現順序；
   全零桶（無任何有效付錢列）不回傳。
3. `SettlementTab.tsx`：entries 改帶幣別——金額入該列幣別桶、手續費非零時另計一 entry 入 TWD 桶；
   渲染改一個幣別一個區塊（標題＝台幣／`currencyLabel（代碼）`，結餘表與結算建議的單位跟著幣別）；
   無可結算列時顯示「尚無可結算的花費。」；說明文字更新。

**不要做**：不保留貪婪法或模式切換；不跨幣別對沖；不在結算頁顯示換匯後參考值；
`settle` 以外的金額純函式（`expenseSubtotal` 等）不動——花費頁的台幣小計/總計/平均照舊換匯。

**驗收**：test／build 全綠；`settle` 既有測試不變（皆單向債務案例、兩法同解）＋新增成對淨額
與 `settleByCurrency` 測試；手動：A 付 1200、B 付 300（台幣、三人均分）→ 結算建議為
B→A 300、C→A 400、C→B 100；外幣列以外幣面額結算、台幣列與手續費以台幣結算，兩區塊分開顯示。
