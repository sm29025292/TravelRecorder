...
| T41 | 新增旅程時強制先輸入旅程名稱與日期 | P2 | ★ | T40（同對話框） | ⏳ 待確認 |
| T42 | 新旅程花費預設帶入常用項目（機票×2／飯店／接駁／換匯／保險） | P2 | ★★ | T40（同建立流程） | ⏳ 待確認 |
...
## T41 新增旅程時強制先輸入旅程名稱與日期（P2・★・依賴：T40）⏳ 待確認

> 狀態：**待擁有者確認後才施工**（2026-09-07 提出）。以下為初步規格，開工前請再過一次「待確認決定」。

**背景／目標**：目前無論「全空」或「複製」，新旅程都是直接建立（名稱給預設值、日期留空），
使用者常忘了補。改為在 T40 的新增對話框裡**先要求輸入旅程名稱與出發／回程日期**，
填完才能建立，減少「無名稱、無日期」的旅程。

**牽涉檔案**：`src/pages/TripList.tsx`（T40 的新增對話框）。

**初步規格**：
1. T40 對話框加三個欄位：旅程名稱（`TextInput`）、出發日期、回程日期（`DateInput`）。
   「全空」與「複製」兩條路徑共用這組欄位。
2. 建立按鈕（「建立空白旅程」／「複製並建立」）在**名稱為空**時 `disabled`。
3. `createTrip` 改為接受使用者輸入的 name／startDate／endDate 寫入 trip，
   不再用「新旅程」預設名、也不再一律留空日期。
4. 複製路徑：名稱預設帶「（來源名）（複製）」讓使用者可改；日期預設空、由使用者填。

**待確認決定（開工前請擁有者拍板）**：
- (a) 日期是否為**必填**？還是只有名稱必填、日期可留空？
      （T40 原拍板是「日期留空待填」；本任務要改成建立時就填，兩者取捨。）
- (b) 是否檢查 `endDate >= startDate`？不合法時如何提示（disabled／alert）？
- (c) 全空旅程的名稱要不要仍給預設字「新旅程」讓使用者改，還是強制清空要求輸入？

**不要做**：不加行事曆選擇器（沿用既有 `DateInput`）；不動 T40 的複製範圍。

**驗收**：build／既有測試全綠；手動：名稱空時無法建立；填完名稱＋日期後建立，
新旅程即帶正確名稱與日期（總覽頁顯示一致）。

---

## T42 新旅程花費預設帶入常用項目（P2・★★・依賴：T40）⏳ 待確認
...
**牽涉檔案**：`src/pages/TripList.tsx`（`createTrip` 建立流程，於同一 transaction 內
`bulkAdd` 這些 `ExpenseItem`）；預設清單建議抽成常數（放 `TripList.tsx` 或新 `src/lib/` 常數檔）。
`ExpenseItem` 型別／schema 不動（沿用既有欄位）。

**初步規格**：
1. 定義預設品項常數陣列（上列 6 項，依序）。
2. `createTrip`（全空與複製兩路徑皆然）在 `db.trips.add` 後，於同一 transaction
   `db.expenses.bulkAdd` 這 6 筆：`tripId` 為新旅程、`item` 為品項名、`sort` 依序遞增、
   金額／手續費／付錢者等欄位留空（沿用「數字 0 顯示空白」慣例）、`currency` 見下方待確認。
3. 交易表加入 `db.expenses`（目前 T40 的交易是 `db.trips, db.members, db.packing`）。

**待確認決定（開工前請擁有者拍板）**：
- (a) 每筆的預設**幣別**：機票／飯店／保險常以台幣付，接駁／換匯可能外幣——
      是全部給 `trip.currencyCode`（外幣）、全部給 `TWD`、還是逐項指定？
- (b) 品項中文名是否照上列文字？（例如要「機票 去程／回程」還是「去程機票」等）
- (c) 複製既有旅程時：既然 T40 不複製來源花費，這 6 筆預設**照常帶入**即可，
      確認不會與任何複製來的花費重複（目前不會，因為不複製 expenses）。
- (d) 是否要讓使用者能關閉此預設（例如對話框加勾選「帶入常用花費項目」），
      還是一律帶入、不需要的自己刪？

**不要做**：不做「花費範本管理 UI」（本任務只是硬編的預設清單）；不動分帳邏輯；
不升 Dexie／不動 schema。

**驗收**：build／既有測試全綠；手動：新建旅程（全空與複製）後花費分頁即出現這 6 筆空白項目、
順序正確、可正常編輯與刪除、台幣小計/總計/平均照常運作。
